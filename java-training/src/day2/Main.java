...
//        t->2, h - > 1, i->2
        ...
        System.out.println(binarySearch(arry, 11));
    ...
    /**
     * @param arr    This is a sorted array in ascending order
     * @param number The value that is being searched for
     * @return The index of the number in arr
     */
    public static int binarySearch(int arr[], int number) {
        ...
        while (high >= low) {
            int mid = (low + high) / 2;
            if (number < arr[mid]) {// that the number lies on the left hand side of the array
                high = mid - 1;
            } else if (number == arr[mid])
                return mid;
            else { // number > arr[mid]  that means the number lies on the right hand side of the array
                // low    mid    high
                low = mid + 1;
            }
        }
